...
export function GradientLayout({ children }: PropsWithChildren) {
  return (
    <div className="relative w-screen h-screen overflow-hidden  bg-gray-50 dark:bg-black ">
      <TopLines />
      <BottomLines />
      <SideLines />
      <TopGradient />
...
const TopGradient = ({ className }: { className?: string }) => {
  return (
    <svg
      ...
      fill="none"
      className={cn(
        'pointer-events-none absolute -left-96 -top-20 hidden h-full w-full dark:block',
        className
      )}
    >
      <path
